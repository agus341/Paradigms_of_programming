...
class Empleado:
    def __init__(self,nombre : int,id,salario_base):
        self.nombre = nombre
        self.id = id
...
class EmpleadoTiempoCompleto(Empleado):
    ...
    def calcular_salario(self):
        ...
        salario = base + (self.bono_anual / 12)
        return salario
...
class EmpleadoMedioTiempo(Empleado):
    ...
    def calcular_salario(self):
        base = super().calcular_salario()
        salario = (base * self.horas_trabajadas) /80
        return salario
...
Empleados = [
    EmpleadoTiempoCompleto("agus",1,100,30),
    EmpleadoMedioTiempo("Tomi",2,200,60)
]

for emp in Empleados:
    emp.informacion()
    print(f"Salario calculado de este mes: {emp.calcular_salario():.2f}\n")
